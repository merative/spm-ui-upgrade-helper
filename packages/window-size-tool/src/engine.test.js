...
describe("applyRules", () => {
  ...
  test("New test", () => {
      const files = ['/Users/luizasalek/Desktop/1/spm-ui-upgrade-helper/packages/window-size-tool/test-data/test-case-1/input/7.uim'];
      const rules = [];
      const expected = false;
      ...
      // ]);
      const parser = new xmldom.DOMParser();
      applyRules(files,rules, sizes, fileio, parser);
