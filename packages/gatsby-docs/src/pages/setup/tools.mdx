...
|SVR_DOUBLE | Maps to a floating point number that corresponds to the Java:tm: primitive data type `double` in a text or text edit field. |
|SVR_FLOAT | Maps to a floating point number that corresponds to the Java:tm: primitive data type `float` in a text or text edit field. |
