...
1. Set up a 7.0.11.0 development environment that contains your custom changes, which provides the input data for the tool. The INPUT_FOLDER for the tool is designed to be the Social Program Management installation directory, which by default is  `C:\IBM\Curam\Development` or `opt/IBM/Curam/Development`. The tool scans the `EJBServer/components` and `webclient/components` folders in the input folder.
