...
2. Load the Docker images:
   ...
   ![Terminal output from docker load command](../../images/run-docker-load.png)

3. Open Docker Desktop and check the images.

   ![Docker Desktop showing the four images](../../images/docker-view-images.png)
...
6. Set the required variables:
  ...
  Where:
   ...
   - `VERSION` is the version of the tool, typically `latest`.
   - `INPUT_FOLDER` is the absolute path to the input files for the tool. Typically, the Social Program Management installation folder, which by default is `C:\IBM\Curam\Development` or `opt/IBM/Curam/Development`.
...
6. Run the Docker compose file.
  ...
  ```
   ...
   ![Docker Desktop showing the four images](../../images/docker-compose.png)

 8. In Docker Desktop, you can see the containers running.

   ![Docker Desktop showing the containers running](../../images/view_running_containers.png)
