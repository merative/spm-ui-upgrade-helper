...
When you have configured your input and output folders with the required permissions, you can run the tools.

Before you begin, remember that `<output folder>` will be wiped, so be careful!
...
1. Start the tool:
    ...
    - Where:
        ...
        - `<output folder>` is the absolute path to your temporary folder. For example, `c:\temp\upgrade` or `/tmp/upgrade`. Run `chmod -R 777 <output folder>`           so that Docker can interact with the output folder.
