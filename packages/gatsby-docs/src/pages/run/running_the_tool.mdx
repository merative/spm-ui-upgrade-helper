...
4. Download the appropriate batch file or shell script for your environment:
...
5. (Linux®️ only) Make the shell script executable with `chmod +x spm-ui-upgrade-helper.sh`

6. Download the docker-compose.yml file:

   Windows ™️

   `curl -L curl -L https://raw.githubusercontent.com/merative/spm-ui-upgrade-helper/main/docker-compose.yml -o docker-compose.yml -o docker-compose.yml`
  
   Linux ®️

   `curl -L curl -L https://raw.githubusercontent.com/merative/spm-ui-upgrade-helper/main/docker-compose.yml -o docker-compose.yml -o docker-compose.yml`

7. Run `chmod -R 777 <output_folder>` so that Docker can interact with the temporary output folder. For example, `C:\temp\upgrade` or `/tmp/upgrade`.
...
1.  Set the required variables and run the batch file or shell script:
