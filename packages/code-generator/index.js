...
const tools = utils.readJson("config/tools.json");
...
const serverJsTemplate = utils.readLines("packages/code-generator/server.js.template").join("\n");
const dummyServerJsTemplate = utils.readLines("packages/code-generator/dummy-server.js.template").join("\n");
tools.forEach(tool => {
  nunjucks.configure();
  ...
  utils.writeLines(outputFile, outputData);
...
const functionsTemplate = utils.readLines("packages/code-generator/functions.ts.template").join("\n");
nunjucks.configure();
...
utils.writeLines(functionsOutputFile, functionsOutputData);
